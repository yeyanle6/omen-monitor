...
/* Generated by Makefile: defines OMEN_PP_API_NEW / _HANDLER / _INTERMEDIATE */
 ...
 * The Makefile inspects the installed kernel header and writes one of three
 * defines to omen_pp_compat.h to select the appropriate code path:
 *
 *   OMEN_PP_API_NEW (>= 6.14):
 *     devm_platform_profile_register(), struct platform_profile_ops with
 *     device-scoped callbacks.
 *
 *   OMEN_PP_API_HANDLER (<= 6.13):
 *     struct platform_profile_handler, platform_profile_register() returning
 *     int, handler-pointer callbacks.
 *
 *   OMEN_PP_API_INTERMEDIATE (Debian 6.12):
 *     struct platform_profile_handler for registration; the header may also
 *     declare a differently-shaped struct platform_profile_ops.
 *
 * On non-NEW kernels, a private hp_wmi_ops type is defined with new-style
 * callbacks and aliased to platform_profile_ops, keeping the driver body
 * unchanged while bridging to the old registration API.
 */

#ifndef OMEN_PP_API_NEW

/*
 * Private ops type with the same interface as 6.14's platform_profile_ops.
 * A distinct name avoids conflicts with whatever the kernel header declares.
 */
struct hp_wmi_ops
{
...
/* Redirect platform_profile_ops declarations in the driver body to hp_wmi_ops. */
#define platform_profile_ops hp_wmi_ops

static const struct hp_wmi_ops *hp_wmi_compat_ops;

#ifdef OMEN_PP_API_HANDLER
/* Bridge to the handler-based platform_profile API (kernel <= 6.13). */
static struct platform_profile_handler hp_wmi_compat_handler;

static int hp_wmi_compat_get(struct platform_profile_handler *pph,
                             enum platform_profile_option *profile)
{
  return hp_wmi_compat_ops->profile_get(NULL, profile);
}
static int hp_wmi_compat_set(struct platform_profile_handler *pph,
                             enum platform_profile_option profile)
{
  return hp_wmi_compat_ops->profile_set(NULL, profile);
}
...
static struct device *devm_platform_profile_register(
    ...
    const struct hp_wmi_ops *ops)
{
  int err;

  hp_wmi_compat_ops = ops;
  hp_wmi_compat_handler.profile_get = hp_wmi_compat_get;
  hp_wmi_compat_handler.profile_set = hp_wmi_compat_set;

  if (ops->probe)
  {
    err = ops->probe(drvdata, hp_wmi_compat_handler.choices);
    if (err)
      return ERR_PTR(err);
  }
  err = platform_profile_register(&hp_wmi_compat_handler);
  if (err)
    return ERR_PTR(err);
  err = devm_add_action_or_reset(dev, hp_wmi_compat_remove_cb, NULL);
  if (err)
    return ERR_PTR(err);
  return (struct device *)&hp_wmi_compat_handler;
}

#else  /* OMEN_PP_API_INTERMEDIATE */
/*
 * Bridge for the Debian 6.12 intermediate API: platform_profile_handler
 * is used for registration, mirroring the OMEN_PP_API_HANDLER path.
 */
static struct platform_profile_handler hp_wmi_compat_handler;

static int hp_wmi_compat_get_int(struct platform_profile_handler *pph,
                                 enum platform_profile_option *profile)
{
  return hp_wmi_compat_ops->profile_get(NULL, profile);
}
static int hp_wmi_compat_set_int(struct platform_profile_handler *pph,
                                 enum platform_profile_option profile)
{
  return hp_wmi_compat_ops->profile_set(NULL, profile);
}
    ...
    const struct hp_wmi_ops *ops)
{
  ...
  hp_wmi_compat_ops = ops;
  hp_wmi_compat_handler.profile_get = hp_wmi_compat_get_int;
  hp_wmi_compat_handler.profile_set = hp_wmi_compat_set_int;
  ...
  if (err)
    return ERR_PTR(err);
  err = devm_add_action_or_reset(dev, hp_wmi_compat_remove_int, NULL);
  if (err)
    return ERR_PTR(err);
  return (struct device *)&hp_wmi_compat_handler;
}
#endif /* OMEN_PP_API_HANDLER */

#endif /* !OMEN_PP_API_NEW */
...
#define zero_if_sup(tmp)                                                       \
  (zero_insize_support ? 0 : sizeof(tmp)) // use when zero insize is required
...
static const char *const omen_thermal_profile_boards[] = {
    "84DA", "84DB", "84DC", "8572", "8573", "8574", "8575", "8600", "8601",
    "8602", "8603", "8604", "8605", "8606", "8607", "860A", "8746", "8747",
    ...
    "8917", "8918", "8949", "894A", "89EB", "8A15", "8A42", "8BAD", "8C58",
    "8BA9", "8BCA", "8E41", "8D2F",
};
...
/* secs_to_jiffies() was removed; fall back to msecs_to_jiffies(). */
#ifndef secs_to_jiffies
#define secs_to_jiffies(x) msecs_to_jiffies((x) * MSEC_PER_SEC)
#endif
 ...
 *       ret = hp_wmi_perform_query(HPWMI_BATTERY_QUERY, HPWMI_READ, buffer, 1,
 * 128)
 */
static int hp_wmi_perform_query(int query, enum hp_wmi_command command,
  ...
  if (obj->buffer.length < sizeof(*bios_return)) {
    pr_warn("query 0x%x returned an invalid buffer (length %u)\n", query,
            obj->buffer.length);
    ret = -EINVAL;
    goto out_free;
  }
...
static int hp_wmi_get_fan_count_userdefine_trigger(void) {
  ...
  ret = hp_wmi_perform_query(HPWMI_FAN_COUNT_GET_QUERY, HPWMI_GM, &fan_data,
                             sizeof(u8), sizeof(fan_data));
  if (ret != 0)
    return -EINVAL;
...
static int hp_wmi_get_fan_speed(int fan) {
  ...
  int ret = hp_wmi_perform_query(HPWMI_FAN_SPEED_GET_QUERY, HPWMI_GM, &fan_data,
                                 sizeof(char), sizeof(fan_data));
...
static int hp_wmi_get_fan_speed_victus_s(int fan) {
  ...
  ret = hp_wmi_perform_query(HPWMI_VICTUS_S_FAN_SPEED_GET_QUERY, HPWMI_GM,
...
static int hp_wmi_read_int(int query) {
  ...
  ret = hp_wmi_perform_query(query, HPWMI_READ, &val, zero_if_sup(val),
                             sizeof(val));
...
static int hp_wmi_get_tablet_mode(void) {
  ...
  tablet_found =
      match_string(tablet_chassis_types, ARRAY_SIZE(tablet_chassis_types),
                   chassis_type) >= 0;
  if (!tablet_found)
    return -ENODEV;

  ret = hp_wmi_perform_query(
      HPWMI_SYSTEM_DEVICE_MODE, HPWMI_READ, system_device_mode,
      zero_if_sup(system_device_mode), sizeof(system_device_mode));
  if (ret < 0)
    return ret;
...
static int omen_thermal_profile_set(int mode) {
  ...
  ret = hp_wmi_perform_query(HPWMI_SET_PERFORMANCE_MODE, HPWMI_GM, &buffer,
                             sizeof(buffer), 0);
...
static bool is_omen_thermal_profile(void) {
  ...
  return match_string(omen_thermal_profile_boards,
                      ARRAY_SIZE(omen_thermal_profile_boards), board_name) >= 0;
}
...
static int omen_get_thermal_policy_version(void) {
  ...
  ret = hp_wmi_perform_query(HPWMI_GET_SYSTEM_DESIGN_DATA, HPWMI_GM, &buffer,
...
static int hp_wmi_fan_speed_max_set(int enabled) {
  ...
  ret = hp_wmi_perform_query(HPWMI_FAN_SPEED_MAX_SET_QUERY, HPWMI_GM, &enabled,
                             sizeof(enabled), 0);
...
static int hp_wmi_set_block(void *data, bool blocked) {
  ...
  ret = hp_wmi_perform_query(HPWMI_WIRELESS_QUERY, HPWMI_WRITE, &query,
                             sizeof(query), 0);
...
static int hp_wmi_rfkill2_refresh(void) {
  ...
  for (i = 0; i < rfkill2_count; i++) {
    ...
    if (num >= state.count || devstate->rfkill_id != rfkill2[i].id) {
      pr_warn(
          "power configuration of the wireless devices unexpectedly changed\n");
      continue;
    }

    rfkill_set_states(rfkill2[i].rfkill, IS_SWBLOCKED(devstate->power),
...
static ssize_t als_store(struct device *dev, struct device_attribute *attr,
  ...
  ret =
      hp_wmi_perform_query(HPWMI_ALS_QUERY, HPWMI_WRITE, &tmp, sizeof(tmp), 0);
  if (ret)
    return ret < 0 ? ret : -EINVAL;
...
static void hp_wmi_notify_object(union acpi_object *obj, void *context) {
  ...
  case HPWMI_CPU_BATTERY_THROTTLE:
    pr_info("Unimplemented CPU throttle because of 3 Cell battery event "
            "detected\n");
    break;
...
#ifdef OMEN_WMI_NOTIFY_U32
static void hp_wmi_notify(u32 value, void *context) {
  struct acpi_buffer response = {ACPI_ALLOCATE_BUFFER, NULL};
  union acpi_object *obj;

  if (ACPI_FAILURE(wmi_get_event_data(value, &response)))
    return;

  obj = response.pointer;
  hp_wmi_notify_object(obj, context);
  kfree(obj);
}
#else
...
static int __init hp_wmi_rfkill_setup(struct platform_device *device) {
  ...
  if (wireless & 0x1) {
    wifi_rfkill = rfkill_alloc("hp-wifi", &device->dev, RFKILL_TYPE_WLAN,
                               &hp_wmi_rfkill_ops, (void *)HPWMI_WIFI);
    if (!wifi_rfkill)
      return -ENOMEM;
  ...
  if (wireless & 0x2) {
    bluetooth_rfkill =
        rfkill_alloc("hp-bluetooth", &device->dev, RFKILL_TYPE_BLUETOOTH,
                     &hp_wmi_rfkill_ops, (void *)HPWMI_BLUETOOTH);
    if (!bluetooth_rfkill) {
      err = -ENOMEM;
  ...
  if (wireless & 0x4) {
    wwan_rfkill = rfkill_alloc("hp-wwan", &device->dev, RFKILL_TYPE_WWAN,
                               &hp_wmi_rfkill_ops, (void *)HPWMI_WWAN);
    if (!wwan_rfkill) {
      err = -ENOMEM;
...
static int __init hp_wmi_rfkill2_setup(struct platform_device *device) {
  ...
  for (i = 0; i < state.count; i++) {
    ...
    rfkill = rfkill_alloc(name, &device->dev, type, &hp_wmi_rfkill2_ops,
                          (void *)(long)i);
    if (!rfkill) {
      err = -ENOMEM;
      goto fail;
...
inline int
omen_thermal_profile_ec_flags_set(enum hp_thermal_profile_omen_flags flags) {
...
static int
platform_profile_victus_get_ec(enum platform_profile_option *profile) {
  int tp;
...
static int
platform_profile_victus_set_ec(enum platform_profile_option profile) {
  int err, tp;
...
static int victus_s_gpu_thermal_profile_set(bool ctgp_enable, bool ppab_enable,
  ...
  ret = victus_s_gpu_thermal_profile_get(&current_ctgp_state,
                                         &current_ppab_state, &current_dstate,
...
static int
platform_profile_victus_s_get_ec(enum platform_profile_option *profile) {
  int ret = 0;
  bool current_ctgp_state, current_ppab_state;
...
static int
platform_profile_victus_s_set_ec(enum platform_profile_option profile) {
...
#ifdef OMEN_PLATFORM_REMOVE_RETURNS_INT
...
static int hp_wmi_hwmon_init(void) {
  ...
  if (ret)
    return ret;
  hwmon =
      devm_hwmon_device_register_with_info(dev, "hp", priv, &chip_info, NULL);
...
static void __init setup_active_thermal_profile_params(void) {
  ...
  if (id) {
    ...
    if (active_thermal_profile_params->ec_tp_offset == HP_EC_OFFSET_UNKNOWN) {
      pr_warn("Unknown EC layout for board %s. Thermal profile readback will "
              "be disabled. Please report this to "
...
static int __init hp_wmi_init(void) {
  ...
  if (hp_wmi_perform_query(HPWMI_HARDWARE_QUERY, HPWMI_READ, &tmp, sizeof(tmp),
                           sizeof(tmp)) == HPWMI_RET_INVALID_PARAMETERS)
